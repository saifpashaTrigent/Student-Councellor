...
async def get_hn_stories(
    limit: int = 5, keywords: List[str] = None, story_type: str = "top"
):
    """
    ...
    for story in stories:
        ...
        if keywords is None or any(
            keyword.lower() in story["title"].lower() for keyword in keywords
        ):
...
async def get_relevant_comments(story_id: int, limit: int = 10):
    ...
    async with aiohttp.ClientSession(
        ...
        comment_details = await asyncio.gather(
            *[fetch_item(session, cid) for cid in comment_ids]
        )
